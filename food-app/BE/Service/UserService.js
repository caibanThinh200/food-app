...
class UserService{
    ...
    static async loginService(req,res,next){
        ...
        if(user){
        if(!data.username ||!bcrypt.compareSync(data.pass,user.Pass)||!data.pass){
            return "Sai tên đăng nhập hoặc mật khẩu";
        }
    
        else{
            const token  = jwt.sign({id:user.UserId,username:user.Username,fullname:user.Fullname,address:user.UserAddress,birth:user.Birth,gmail:user.Gmail},JWT_SECRET_KEY,{expiresIn:60*60*8});
    ...
    static async getUserInfoService(req,res,next){
        try{
            ...
            if(token){
                ...
                let data = await querryBuilder("users").where("UserId",checkToken.id).select();
